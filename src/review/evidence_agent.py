...
def _resolve_disk_source(
    root: Path,
    item_path: str,
    source_rel_path_by_logical_path: Optional[Dict[str, object]] = None,
) -> Path:
    ...
    indexed ``rel_path`` uses ``::``. Prefer the immutable index-owned mapping
    when present, then fall back to the conventional ``__`` filename form.
    Every candidate is checked to remain inside ``root``.
    """
    root_resolved = root.expanduser().resolve()
    candidates: List[str] = []
    logical_key = _clean_rel_path(item_path)
    if isinstance(source_rel_path_by_logical_path, dict):
        mapped = source_rel_path_by_logical_path.get(logical_key)
        if isinstance(mapped, str) and _clean_rel_path(mapped):
...
def build_evidence_tools(
    ...
    *,
    ...
    def ocr_attachment(
        ...
        try:
            source = _resolve_disk_source(
                Path(root_raw),
                item_path,
                source_rel_path_by_logical_path=attachments.get(
                    "source_rel_path_by_logical_path"
                ),
            )
        except (OSError, ValueError):
            return _record(
